...
export default function UdaipurTripCard({
  udaipurData,
  formatCurrency,
  onEditTripItem,
  onAddTripItem,
    ...
    <div>
      ...
      <div className="flex items-center justify-between border-b border-brand-border pb-2.5 mb-3.5 flex-wrap gap-4">
        <div className="flex items-center gap-3">
          <div>
            ...
            <span className="text-xs text-brand-muted">
              Travel expense summary and split details
            </span>
          </div>
          {editable && (
            <button
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {/* Custom Tab Toggles */}
          <div className="grid grid-cols-2 md:flex rounded-lg border border-brand-border bg-brand-bg/50 p-1 gap-1 w-full">
            {[
              ...
              <button
                ...
                className={`w-full px-3 py-1.5 text-xs font-semibold rounded-md transition-all cursor-pointer ${
                  activeTab === tab.id
                    ? "bg-brand-accent text-white shadow-xs"
                    : "text-brand-muted hover:text-brand-text"
                }`}
      ...
      {activeTab === "overview" && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-7 grid grid-cols-2 gap-2.5">
            <div className="bg-brand-bg/60 border border-brand-border/60 p-3 rounded-xl text-center flex flex-col justify-center gap-1 col-span-2">
              <span className="text-xs font-medium text-brand-muted">
                Grand Total for {selectedUser === "me" ? "Me" : "Virat"}
              </span>
              <span className="text-2xl font-extrabold text-brand-accent">
                {formatCurrency(userGrandTotal)}
              </span>
              <span className="text-[10px] text-brand-muted">
                Per Person Share ({formatCurrency(splitAmount)}) + Personal
                Spend ({formatCurrency(userPersonalSpend)})
              </span>
            </div>
          ...
          <div className="lg:col-span-5 flex flex-col items-center justify-center p-3 bg-brand-card border border-brand-border rounded-xl">
            <h4 className="text-xs font-bold text-brand-text mb-3 text-center">
            ...
            >
              <canvas
                ref={canvasRef}
                aria-label="Shared expenses categories chart"
                role="img"
              />
            </div>
                ...
                {activeList.map((item, idx) => (
                  <tr
                    ...
                    onClick={() =>
                      ...
                      onEditTripItem({
                        ...item,
                        type: activeTab === "shared" ? "combined" : activeTab,
                      })
                    }
